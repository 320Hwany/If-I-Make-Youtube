...
class QueryVideoInfoCacheByIdTest extends ServiceTest {

    @Autowired
    private QueryVideoInfoCacheById queryVideoInfoCacheById;

    @Test
    @DisplayName("id와 일치하는 동영상 정보가 없다면 동영상 캐시 정보를 가져올 수 없습니다")
    void queryVideoInfoCacheByIdFail() {
        // expected
        assertThatThrownBy(() -> queryVideoInfoCacheById.query(9999L))
    ...
    void queryVideoInfoCacheByIdSuccess() {
        ...
        // when
        VideoInfoCache cache = queryVideoInfoCacheById.query(videoInfo.getId());
